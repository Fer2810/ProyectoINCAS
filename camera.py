...
predictor = dlib.shape_predictor(".venv/Lib/site-packages/dlib/models/shape_predictor_68_face_landmarks.dat")
facial_recognition_model = dlib.face_recognition_model_v1(".venv/Lib/site-packages/dlib/models/dlib_face_recognition_resnet_model_v1.dat")
...
def start_camera():
    ...
    if not camera_running:
        ...
        cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)  # Limitar el tamaño del buffer
        camera_running = True
        processing = False
        student_info = None
        descriptor = None
...
def generate():
    ...
    while camera_running:
        try:
            ...
            # Función para reiniciar los valores después de 5 segundos
            if not processing:
                threading.Timer(5, reset_values).start()
                processing = True
            ...
            print(f"Error: {e}")
            continue  # Manejar cualquier otro error y continuar con el siguiente frame
